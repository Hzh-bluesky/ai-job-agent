...
public class DeepSeekAIService implements LLMService {
    ...
    public LLMResponse chat(LLMRequest request) {
        ...
        String model = StringUtils.hasText(deepSeek.getModel()) ? deepSeek.getModel() : "deepseek-chat";
        Double temperature = deepSeek.getTemperature() == null ? 0.2 : deepSeek.getTemperature();
        Integer maxTokens = deepSeek.getMaxTokens() == null ? 8192 : deepSeek.getMaxTokens();

        if (!StringUtils.hasText(deepSeek.getApiKey())) {
            return failed(model, "DeepSeek API Key未配置，请设置DEEPSEEK_API_KEY", "CONFIG_ERROR");
        }

        try {
            String url = normalizeBaseUrl(deepSeek.getBaseUrl()) + "/chat/completions";
            ...
            ResponseEntity<String> responseEntity = buildRestTemplate(deepSeek.getTimeoutSeconds())
            ...
            return LLMResponse.builder()
                    ...
                    .errorMessage("DeepSeek调用失败：" + ex.getStatusCode())
                    .build();
        } catch (RestClientException ex) {
            return failed(model, "DeepSeek网络调用失败：" + ex.getMessage(), clientFailureType(ex.getMessage()));
        } catch (Exception ex) {
            return failed(model, "DeepSeek响应解析失败：" + ex.getMessage(), "RESPONSE_PARSE_ERROR");
        }
    }
    ...
    private String normalizeBaseUrl(String baseUrl) {
        String value = StringUtils.hasText(baseUrl) ? baseUrl.trim() : "https://api.deepseek.com";
